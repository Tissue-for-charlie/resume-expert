...
2. **行业矩阵扩展**是最高频的贡献方向——如果你要添加新行业，请按现有矩阵格式提供：
   ...
   - Step 4 改写倾斜规则
   - 评分权重调整（如适用）
3. 避免添加只有你个人需要的硬编码规则——优先扩展矩阵而非修改核心逻辑
...
- SKILL.md 是核心文件，保持其可读性——超过 600 行时考虑抽出独立文档
...
- [ ] SKILL.md 行数是否控制在合理范围（当前 706 行，目标 ≤720）？如果新增内容超过 50 行，是否应考虑抽出到 `docs/` 下？
- [ ] 如果新增了行业，是否同时更新了评分权重映射表（模式 B Step 2）和 Step 4 改写倾斜列表？
