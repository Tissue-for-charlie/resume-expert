...
name: resume-expert
version: 1.3.0
description: 当你需要制作、修改、评审简历或针对目标岗位做 JD 匹配度扫描时触发此技能——包括从零生成简历、优化现有简历、从对话历史中自动提取信息来完善简历、以及对已有简历做目标 JD 命中率分析。支持中英文、全行业（14 个内置行业矩阵 + 未覆盖行业通用策略自动推导）、全职级（实习/校招/社招），默认输出为 docx-js 生成的 Word 文档。
---
...
| [`scoring-golden-tests.md`](references/scoring-golden-tests.md) | **评分金标准测试集**：70 份校准简历，14 行业 × 5 分数段全覆盖，含 🌐 Web 搜索增强校准 |
