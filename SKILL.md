...
name: resume-expert
version: 1.4.0
description: 当你需要制作、修改、评审简历或针对目标岗位做 JD 匹配度扫描时触发此技能——包括从零生成简历、优化现有简历、从对话历史中自动提取信息来完善简历、以及对已有简历做目标 JD 命中率分析。支持中英文、全行业（15 个内置行业矩阵 + 未覆盖行业通用策略自动推导）、全职级（实习/校招/社招），支持 DOCX / HTML / LaTeX / Markdown 多格式输出，内置内容质量自检机制。
---
...
**模式路由**：触发后根据用户意图自动选择模式——只说"做简历" → 模式 A；说"看看简历" → 模式 B；说"改这段" → 模式 C；说"匹配度"/"这个 JD"/附带 JD 文本 → 模式 D。
...
| **娱乐/演艺** | 代表作品、全网影响力数据（播放量/粉丝数/榜单排名）、荣誉奖项、演出经历、社交平台账号（微博/流媒体/短视频） | 作品集链接（B站/网易云/抖音）放头部；翻唱/翻跳/自媒体数据为王 | 专业能力证明（院校/师承/比赛成绩）+ 代表作品 + 传播数据 | 全网影响力数据（播放量/粉丝数/榜单登顶次数）、荣誉奖项、演出/巡演规模、商业合作层级 |
...
**未覆盖行业的通用策略**：
1. 识别行业核心凭证（证书？作品集？项目数据？学历？粉丝/传播数据？）
...
在向用户提任何问题之前，**必须**完成以下扫描（1-3 项并行执行，各路径独立容错）。但在开始扫描前先做**空目录快速判定**：

**空目录快速判定**：检查当前工作目录是否为空或仅含 `.git`/空子目录/无关文件（如 `.claude/`、`node_modules/`）。若为空**且**用户首条消息未附带任何简历文本（无 ≥100 字粘贴内容、无文件附件），则 **跳过所有本地/GitHub/依赖扫描**，直接进入 Step 1（用户显然是新建文件夹来生成简历的）。
...
| 序号 | 维度 | 收集内容 | 自动跳过条件 |
|------|------|----------|-------------|
| 1 | 基本信息 | 姓名 / 电话 / 邮箱 / 城市。**联系方式字段按行业类型自适应**：技术行业追问 GitHub/LinkedIn；娱乐/演艺行业追问微博/流媒体平台（B站/网易云/抖音）/ Instagram；设计行业追问作品集平台（站酷/Behance）；学术行业追问学术数据库主页 | — |
...
| 4 | 项目经历 | 逐个确认，先列出已知再问遗漏。**娱乐/演艺行业改为「演出/项目经历」**：演唱会/综艺/音乐节/影视OST等 | — |
| 5 | 工作/实习 | 公司名 / 岗位 / 时间段 / 主要产出 | — |
| 6 | 竞赛/证书 | 比赛名称 + 奖项 / 语言成绩 / 专业证书。**娱乐/演艺行业改为「荣誉奖项/证书」**：音乐奖项/声乐等级/语言证书 | 信息 ≤4 维度时自动跳过 |
| 7 | 作品/博客 | GitHub / 个人网站 / 技术博客 / 设计作品集。**娱乐/演艺行业改为「代表作品/作品集」**：专辑/单曲/OST/B站作品/演唱会录像 | — |
...
**娱乐/演艺行业默认模块布局**：头部（含社交/作品平台链接）→ 联系方式 → 个人简介/定位 → 影响力数据（播放量/粉丝数/奖项数）→ 代表作品 → 荣誉奖项 → 工作/演出经历 → 教育背景。见 industry-matrix.md 完整展开。
...
**生成前**：根据经验值预估页数并告知用户。同时**询问用户输出格式偏好**——未指定时默认 DOCX。支持以下格式：

#### 格式选择

| 格式 | 适用场景 | 生成方式 |
|------|----------|----------|
| **DOCX**（默认） | 官网投递/HR 系统/ATS 扫描 | docx-js（Node.js）生成 |
| **HTML** | 网页版投递/个人作品集页/打印 PDF | 手写 HTML + CSS（注意打印样式 `@media print`） |
| **LaTeX** | 学术/技术岗位/需精确排版的正式投递 | `.tex` 源码 + xelatex 编译为 PDF |
| **Markdown** | 快速预览/零依赖保底/非 Claude Code 平台 | 格式精美的纯文本 Markdown |

#### DOCX 格式标准

A4 页面 / 0.6" 页边距 / 微软雅黑+Arial（`font: { ascii: "Arial", eastAsia: "微软雅黑" }`）/ 姓名 16pt · 标题 11pt · 正文 9pt / 颜色 #333333/#1A1A1A/#666666/#2B579A / 章节标题下方 1px 分隔线。

强制 API：`numbering config` + `format: "bullet"`（v8）或 `LevelFormat.BULLET`（v9+）；所有 TextRun 显式设 font；超链接用 ExternalHyperlink 包裹。

#### HTML 格式标准

- 使用内联 `<style>`（单文件自包含，无需外链）
- 必须包含 `@media print` 打印样式
- 字体用系统安全字体栈（`Noto Sans SC` / `微软雅黑` 等）
- 配色：深色/浅色/清新风格均可，保持专业统一
- 响应式设计：至少适配桌面 + 手机两种宽度
- 内容布局：线性流式单栏（ATS 兼容），禁止表格/双栏

#### LaTeX 格式标准

- 使用 `xelatex` 编译，`fontspec` + `xeCJK` 处理中文字体
- 字体指定落笔系统实际存在的字体名（避免 Missing character 警告）
- 中文字体通过 `\setCJKmainfont` / `\setCJKsansfont` 配置；Bold 权重使用 `AutoFakeBold` 或明确指定 BoldFont
- 模板尽量简洁：一个 `.tex` 文件即用，不宜分包

> 📖 [`references/docx-spec.md`](references/docx-spec.md) — DOCX 完整排版参数、bullet 版本检测代码、高发坑点
...
**内容质量自检**（**全格式适用，生成后立即执行**）：

| 检查项 | 触发条件 | 处理方式 |
|--------|----------|----------|
| 联系方式完整性 | 缺失电话/邮箱/社交号（按行业判断必要项） | 🔴 提示用户补充 |
| 荣誉奖项为空 | 娱乐/演艺/学术/法律等行业奖项模块为空 | 🟡 建议补充核心奖项 |
| 代表作品为空 | 娱乐/演艺/设计/媒体行业作品列表为空 | 🟡 建议补充代表作品 |
| 量化数据覆盖 | 每段经历是否含至少 1 个数字（年份除外） | 🟡 建议补充量化产出 |
| 量化数据一致性 | 如"现象级"/"破纪录"等定性描述无对应数据支撑 | 🟡 建议补充支撑数据 |
| 硬性成就遗漏 | 明显可查的知名奖项/作品未出现在简历中 | 🟡 提示考虑纳入 |

### Step 7：生成失败回退

1. 诊断修复后重试（最常见：`npm install docx`）
2. 切换到未被尝试过的格式重试（如：DOCX 失败 → 尝试 python-docx 回退 / HTML / LaTeX）
3. **最终保底**：输出格式精美的 Markdown 版本——无需任何依赖即可获得可用简历

每次回退时告知用户当前层级及局限性。回退链完整路径：`docx-js → python-docx → HTML → LaTeX → Markdown`，任一层失败自动降级。
...
> 📖 [`references/scoring-golden-tests.md`](references/scoring-golden-tests.md) — 70 份金标准简历 + 专家评分明细，14 行业全覆盖 × 5 分数段，含 Web 搜索增强校准
...
**行业 × 权重映射**：14 个行业各有独立权重（如互联网/科技关键词升至 30、金融/法律资质升至 25、设计/外企加分项升至 25-30）。详见 [`references/jd-matching.md`](references/jd-matching.md) 行业权重映射表。
...
### 技术约束
- **T1. 输出规范**：DOCX 输出时所有 TextRun 显式设 font（中英分离），Bullet 只用 numbering config，超链接用 ExternalHyperlink；HTML 输出时使用线性流式单栏布局并确保 `@media print` 打印样式；LaTeX 输出时使用 `xelatex` + `xeCJK` 处理中文，确保系统字体存在。GitHub/网站/作品集链接优先嵌入标题行
...
### Q4：怎么添加新行业？
告诉 AI 目标行业即自动走通用策略推导或对标最相近的矩阵行。贡献到矩阵（目前 15 个内置行业）见 [CONTRIBUTING.md](CONTRIBUTING.md)。
...
| [`industry-matrix.md`](references/industry-matrix.md) | 行业×职级全量策略（15 个行业）、差异化结构、改写倾斜全表、作品集链接处理 |
| [`scoring-system.md`](references/scoring-system.md) | 评分权重映射（15 个行业）、分数锚点、**同岗位基准对比**、反馈格式、评分前校准流程 |
| [`scoring-golden-tests.md`](references/scoring-golden-tests.md) | **评分金标准测试集**：70 份校准简历，15 行业 × 5 分数段全覆盖，含 🌐 Web 搜索增强校准 |
| [`jd-matching.md`](references/jd-matching.md) | 🆕 **JD 匹配度扫描**：JD 三层信息提取、能力等级映射、关键词归一化、行业权重映射、红旗检测、报告格式 |
| [`rewrite-rules.md`](references/rewrite-rules.md) | 项目描述重写对照表、STAR 框架、英文动词库（含表演/音乐分类）、常见错误 |
...
| [`docx-spec.md`](references/docx-spec.md) | 排版参数大全（DOCX / HTML / LaTeX）、bullet v8/v9 检测、高发坑点、验证清单 |
